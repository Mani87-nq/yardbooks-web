...
export default function GridSettingsPage() {
  ...
  const gridSettings = settings.gridSettings || DEFAULT_GRID_SETTINGS;
  ...
  const shortcutProductIds = useMemo(
    () => new Set(gridSettings.shortcuts.map((s) => s.productId)),
  ...
  const moveShortcut = (index: number, direction: 'up' | 'down') => {
    const newShortcuts = [...gridSettings.shortcuts];
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Color Shortcuts
                <Badge variant="info">{gridSettings.shortcuts.length}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* Current Shortcuts */}
              {gridSettings.shortcuts.length > 0 ? (
                    ...
                    .map((shortcut, index) => {
                      ...
                      return (
                        <ShortcutTile
                          ...
                          isFirst={index === 0}
                          isLast={index === gridSettings.shortcuts.length - 1}
                        />
                      );
                    })}
                </div>
