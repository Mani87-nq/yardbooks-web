...
export default function SessionHistoryPage() {
  ...
  const filteredSessions = useMemo(() => {
    ...
    switch (dateFilter) {
      case 'today':
        filtered = filtered.filter(s => new Date(s.openedAt) >= today);
        break;
      case 'week':
        filtered = filtered.filter(s => new Date(s.openedAt) >= weekAgo);
        break;
      case 'month':
        filtered = filtered.filter(s => new Date(s.openedAt) >= monthAgo);
        break;
    }
  ...
  const summaryStats = useMemo(() => {
    ...
    const totalSales = closedSessions.reduce((sum, s) => sum + s.netSales, 0);
    const totalVariance = closedSessions.reduce((sum, s) => sum + (s.cashVariance || 0), 0);
    const sessionsWithVariance = closedSessions.filter(s => s.cashVariance && s.cashVariance !== 0).length;
  ...
  const cashierPerformance = useMemo(() => {
    ...
    sessions.filter(s => s.status === 'closed').forEach(session => {
