        ...
        <CardContent>
          {transfers.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-400 mt-1">Create a transfer to move inventory between locations.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-semibold text-gray-900 text-sm">{transfer.transferNumber}</span>
                  ...
                  <div className="text-right">
                    <p className="font-medium text-gray-900 text-sm">{fc(transfer.totalValue)}</p>
                    <p className="text-xs text-gray-500">{formatDate(transfer.createdAt)}</p>
                  </div>
                </button>
              ))}
            </div>
                  ...
                  {transferItems.map((item) => (
                    <div key={item.productId} className="flex items-center gap-3 p-2 bg-gray-50 rounded-lg">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500">{item.sku} · {fc(item.unitCost)} / {item.uomCode}</p>
                      </div>
                      <input
                        type="number"
                        min={1}
                      ...
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-red-500 rounded hover:bg-red-50"
                      >
            ...
            <ModalBody>
              <div className="space-y-5">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-500">{formatDate(showDetailModal.createdAt)}</span>
                </div>
                ...
                <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                  <div className="flex-1 text-center">
                    ...
                    <p className="font-medium text-gray-900 text-sm">{showDetailModal.fromWarehouse?.name}</p>
                    <p className="text-xs text-gray-500">{showDetailModal.fromWarehouse?.code}</p>
                  </div>
                  <ArrowRightIcon className="w-5 h-5 text-emerald-500" />
                  <div className="flex-1 text-center">
                    <BuildingStorefrontIcon className="w-6 h-6 mx-auto mb-1 text-emerald-500" />
                    <p className="font-medium text-gray-900 text-sm">{showDetailModal.toWarehouse?.name}</p>
                    <p className="text-xs text-gray-500">{showDetailModal.toWarehouse?.code}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {[
                    ...
                    <div key={event.label} className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-400 ml-auto">{formatDate(event.date!)}</span>
                    </div>
                  ))}
                </div>
                ...
                {showDetailModal.notes && (
                  <div>
                    ...
                    <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">{showDetailModal.notes}</p>
                  </div>
                )}
