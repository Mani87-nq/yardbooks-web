/**
 * POST /api/modules/salon/appointments/[id]/start — Mark appointment as in-progress
 */
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!['BOOKED', 'CONFIRMED'].includes(appointment.status)) {
      return badRequest(
        `Cannot start appointment with status "${appointment.status}". Only BOOKED or CONFIRMED appointments can be started.`
      );
    }
    ...
    return internalError(error instanceof Error ? error.message : 'Failed to start appointment');
  }
}
