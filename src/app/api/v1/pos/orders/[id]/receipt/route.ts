/**
 * POST /api/v1/pos/orders/[id]/receipt — Send receipt email for a POS order.
 *
 * Sends a branded receipt email to the customer. Accepts an optional `email`
 * override; otherwise uses the order's `customerEmail`.
 * Updates the order's `receiptEmail` field for audit trail.
 */
...
/** Map POS payment method codes to human-readable labels. */
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    // Determine primary payment method (most recent completed payment)
    ...
    // Update the order's receipt email field for audit trail
